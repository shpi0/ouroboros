...
### v6.4.0 -- Web Interface (MINOR)
- FastAPI backend on localhost:8080 with three sections: Chat (WebSocket), Stats (Chart.js), Settings
- mTLS protection via nginx — only certificate holder can access
- `scripts/gen_mtls_certs.sh` — generates CA, server cert, client .p12 for browser import
- `scripts/nginx_ouroboros.conf` — ready nginx config for ouroboros.iarovoivv.com
- Two-level LLM (local qwen3.5 + OpenRouter fallback) for channel buffer classification

### v6.3.0 -- Channel Buffer (MINOR)
- Pyrogram-based smart forwarding to Telegram buffer channel
- LLM post classification with local Ollama + OpenRouter fallback
- Daily scheduler (06:00 and 18:00 MSK) for buffer auto-fill
- Post cleaning: footer removal, toxic text rephrasing
