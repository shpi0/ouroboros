# ============================
# Ouroboros — VPS runtime launcher (entry point for self-hosted deployments)
# ============================
# Mirror of colab_launcher.py, adapted for bare-metal/VPS:
#   - no `google.colab` imports; secrets come from env (systemd EnvironmentFile or `source .env`)
#   - DRIVE_ROOT / REPO_DIR come from env (OUROBOROS_STATE_DIR / OUROBOROS_REPO_DIR)
#   - no Drive FUSE mount; state persists on local disk
#
# Upstream parity: kept structurally identical to colab_launcher.py so that
# future upstream changes can be ported with a diff.
...
# Workers use multiprocessing start_method=fork (see supervisor/workers.py).
# Fork is Linux-only in practice — fail fast with a clear message elsewhere.
assert sys.platform.startswith("linux"), (
    "Ouroboros supervisor requires Linux (workers use multiprocessing start_method=fork)."
)
...
# ----------------------------
# 1) Secrets + runtime config (env-only on VPS)
# ----------------------------
def get_secret(name: str, default: Optional[str] = None, required: bool = False) -> Optional[str]:
    v = os.environ.get(name, default)
    if required:
        assert v is not None and str(v).strip() != "", f"Missing required env var: {name} (check .env / systemd EnvironmentFile)"
    return v
...
# Robust TOTAL_BUDGET parsing — handles stray whitespace/punctuation.
try:
    import re
...
assert GITHUB_USER and str(GITHUB_USER).strip(), "GITHUB_USER not set (check .env)."
assert GITHUB_REPO and str(GITHUB_REPO).strip(), "GITHUB_REPO not set (check .env)."
...
# ----------------------------
# 2) Persistent storage paths (local disk, no Drive mount)
# ----------------------------
_state_env = get_cfg("OUROBOROS_STATE_DIR")
_repo_env = get_cfg("OUROBOROS_REPO_DIR")
assert _state_env, "OUROBOROS_STATE_DIR not set (where to persist state/logs/memory)."
assert _repo_env, "OUROBOROS_REPO_DIR not set (where the git working copy lives)."

DRIVE_ROOT = pathlib.Path(_state_env).resolve()
REPO_DIR = pathlib.Path(_repo_env).resolve()

# ouroboros/tools/health.py reads DRIVE_ROOT env (with a /content/... fallback) —
# propagate it so tools running in worker subprocesses see the right path.
