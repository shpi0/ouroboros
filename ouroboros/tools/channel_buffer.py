...
_LLM_SYSTEM_PROMPT = (
    ...
    "- СТРОГО ЗАПРЕЩЕНО (безусловный отказ, ok=false):\n"
    "  * Любые новости об атаках на территорию России: обстрелы городов РФ дронами/ракетами ВСУ\n"
    "  * Пожары, взрывы, разливы нефти на российских объектах, вызванные атаками\n"
    "  * Экологические катастрофы/бедствия от действий ВСУ на территории РФ\n"
    "  * Жертвы среди мирного населения РФ\n"
    "  * Атаки БПЛА/дронов/ракет на российские регионы, предприятия, инфраструктуру\n"
    "  * ТЕСТ: если пост описывает ущерб/последствия для России — это ok=false, даже если тон 'нейтральный' или 'информационный'\n"
    "  * Пример запрещённого поста: 'Ликвидация экологического бедствия в Туапсе, вызванного ударами украинских беспилотников' — ok=false\n"
    "  * Даже если пострадавших нет или тон фактологический — такие новости НЕ публикуем\n"
...
def _is_negative_russia_news(text: str) -> bool:
    ...
    attack_patterns = [
        ...
        r'удар[а-я]*\s+(украинск|вражеск)[а-я]*\s+беспилотник',
        r'беспилотник[а-я]*\s+(вс[уу]|украин[а-я]*)',
        r'атак[а-я]*.{0,40}(нефт|завод|промышленн|инфраструктур)',
        r'(пожар|разлив|взрыв).{0,50}(от\s+атак|после\s+атак|в\s+результате\s+атак|вызванн)',
        r'разлив\s+(нефт|топлив)',
        r'экологическ[а-я]*\s+(катастроф|бедстви)',
        r'(первая|повторная|вторая)\s+атак[а-я]*',
        r'дрон[а-я]*.{0,30}(нефт|завод|промышленн)',
        r'нефтеперераб.{0,20}(атак|обстрел|удар)',
        r'загрязнен[а-я]*.{0,30}(море|реку?|воздух)',
        r'российск[а-я]*.*предприят',
    ]
    ...
    russia_regions = [
        ...
        'в курске', 'в воронеже',
        'туапсе', 'в туапсинск', 'в самаре', 'в нижнем новгороде',
        'в новороссийске', 'в сочи', 'в краснодаре',
        'на объект',
    ]
    ...
    # Комбинированный блок: широкие признаки атаки + регион + последствие (пожар/разлив/взрыв/загрязнение)
    broad_attack_kw = ['атак', 'удар', 'обстрел', 'беспилотник', 'дрон', 'ракет', 'всу', 'украинск']
    consequence_kw = ['пожар', 'разлив', 'взрыв', 'загрязнен']
    has_broad_attack = any(kw in low for kw in broad_attack_kw)
    has_fire_or_spill = any(kw in low for kw in consequence_kw)

    # Блокируем если: (атака + регион) ИЛИ последствие атаки ИЛИ экокатастрофа
    # ИЛИ широкий сигнал атаки + регион + последствие одновременно
    return (has_attack and has_region) or has_consequence or has_eco or (has_broad_attack and has_region and has_fire_or_spill)
